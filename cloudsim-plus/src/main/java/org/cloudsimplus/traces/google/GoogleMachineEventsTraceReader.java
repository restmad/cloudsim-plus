...
public final class GoogleMachineEventsTraceReader extends GoogleTraceReaderAbstract<Host> {
    ...
    /**
     * Process the {@link #getFilePath() trace file} creating a List of {@link Host}s
     * described in the file.
     *
     * <p>It returns the List of {@link Host}s that were available at timestamp 0 inside the trace file.
     ...
     *
     * <p>
     * The list of returned Hosts is not added to any Datacenter. The developer creating the simulation
     * must add such Hosts to any Datacenter desired.</p>
     *
     * @return the List of {@link Host}s that were available at timestamp 0 inside the trace file.
     */
    @Override
    ...
    protected Host createHostFromTraceLine() {
        final MachineEvent event =
                new MachineEvent()
                    .setCpuCores(FieldIndex.CPU_CAPACITY.getValue(this))
