...
public final class GoogleTaskEventsTraceReader extends GoogleTraceReaderAbstract<Cloudlet> {
    ...
    /**
     * Process the {@link #getFilePath() trace file} creating a List of {@link Cloudlet}s
     * described in the file. <b>Each created Cloudlet is automatically submitted to its respective
     * broker.</b>
     *
     * <p>It returns the List of all submitted {@link Cloudlet}s at any timestamp inside the trace file
     * (the timestamp is used to delay the Cloudlet submission).
     * </p>
     *
     * @return the List of all submitted {@link Cloudlet}s for any timestamp inside the trace file.
     * @see #getBrokers()
     */
    @Override
    ...
    private Cloudlet addCloudletStatusChangeEvents(final CloudSimEvent statusChangeSimEvt, final TaskEvent taskEvent){
        ...
        * by sending a request message to the broker.*/
        ...
        by the researcher is the only one that actually know how to create a Cloudlet
        from the trace line.
    ...
    /**
     * Gets the collection of brokers created according to the username from the trace file,
     * representing a customer.
     * @return
     */
    public Collection<DatacenterBroker> getBrokers() {
        return brokersMap.values();
    }
    ...
    /**
     * Sets the simulation instance that the created tasks and brokers will belong to.
     * @param simulation the simulation instance to set
     */
    public void setSimulation(final CloudSim simulation) {
